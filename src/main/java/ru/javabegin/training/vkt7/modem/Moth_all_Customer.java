 ...
 */
@Component
public class Moth_all_Customer extends EventListener{
 //SerialPort serialPort; /*Создаем объект типа SerialPort*/
    ...
    public Operation operation3;
    public Operation operation4;
...
/* public volatile static String data2;
   ...
   /*private static int[][] requests = {
    ...
    public String mothly_all_cycle(List<Customer> customerList,CustomerService customerService, OperationService operationService, Date date, int type ) throws InterruptedException, TimeoutException, ExecutionException, SerialPortException {
        ...
        Map<Timestamp, List<Measurements> > total_current_hashMap = new HashMap<>();
...
for (int countCustomer=0;countCustomer<customerList.size();countCustomer++){
    ...
    System.out.println("FirstName()= = "+ tel);
 ...
 */  ////////////////////////////////////////////////////////////////////////////////
            ...
            /**
             * Подготавливаем время для измерений
            ...
            //// получаем дату (пред)предыдущего месяца
            LocalDateTime lastMoth_2 = ldt.minusMonths(2).with(TemporalAdjusters.lastDayOfMonth());
            ////переводим в строку для работы
            ...
            List<Integer> type_meas =new ArrayList<>();
...
for (int y=0; y<type_meas.size(); y++) {
...
////(!)(!)(!)(!)//////////////////////type=1 /////////////////////////////////////////////////////////////////////////////////////////
    ...
    //List<String> ff_n=new ArrayList<>();
    System.out.println("\nФормируем новый type= "+ type_meas.get(y) + " 3F FD Запрос на запись ТИПА значений");
     ...
     */
    command_3FFD = null;
    command_3FFD = send10Service.s_3FFD(number, type_meas.get(y));
    ...
    /**
     * Делаем перебор даты для месячных значений
     *
     * Месяц - 1
     * Месяц - 2
     *
    ...
    for (int m = 0; m < date_dd_MM_UU_HH.size(); m++) {
        ...
        System.out.println("\nФормируем 3F FB  " + date_dd_MM_UU_HH.get(m) + "      4.4 Запрос на запись даты");


        List<String> command_3FFB = send10Service.s_3FFB(number, date_dd_MM_UU_HH.get(m));
        ...
        System.out.println("\n посылаем запрос 3F FB" + date_dd_MM_UU_HH.get(m) + "  4.4 Запрос на запись даты.");
        Thread.sleep(5000);
        serialPort.writeIntArray(request);
        ...
        while (step == 20) {
            if (data2.contains("3F FB ")) {
                t = 1;
                //System.out.println("После запроса 3F FF_n ");
                System.out.println("Запрс 3F FB;" + date_dd_MM_UU_HH.get(m) + " data2 = " + data2);
            ...
            if (t == 2) {
                ...
                if (repeat == 6) {
                    ...
                    System.out.println("\n Ответ не поступил." + date_dd_MM_UU_HH.get(m) + " Ошибка по таймауту.");
                    System.out.println("\n error=22.3F FB TimeOut");
                    error = 22;
                    stop = false;
                }

                System.out.println("\n Ответ не поступил." + date_dd_MM_UU_HH.get(m) + " Ошибка по таймауту. Повторяем запрос");
                serialPort.writeIntArray(request);
                t = 0;
                executor.submit(callable(6));
            }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        System.out.println("\n Формируем запрос 3F FE МЕСЯЧНЫЕ чтение " + date_dd_MM_UU_HH.get(m));
        ff_2 = null;
        ...
        //System.out.println("\n request size = "+ request.length);
        Thread.sleep(5000);
        System.out.println("\nЖдем получения данных МЕСЯЧНЫЕ после команды 3F FE   " + date_dd_MM_UU_HH.get(m));
        data2 = "";
        step = 21;
        recieve_all_byte = 0;
        t = 0;
 ...
 */
        ...
        while (recieve_all_byte == 0) {
            if (t == 2) {
                ...
                if (repeat == 6) {
                    ...
                    System.out.println("\n Ответ не поступил 3F FE(МЕСЯЧНЫЕ). Ошибка по таймауту.");
                    System.out.println("\n error=23.3F FE TimeOut");
                    error = 23;
                    stop = false;
                }
                System.out.println("\n Ответ на 3F FE  " + date_dd_MM_UU_HH.get(m) + " не поступил. Ошибка по таймауту. Повторяем запрос");
                temp = "";
                z = 0;
                t = 0;
                data2 = "";
        ...
        System.out.println("\nДанные 3F FE (МЕСЯЧНЫЕ ) поступили.");
        System.out.println("Ожидаем обработку принятых данных 3F FE ");
        ...
        System.out.println("Дата для МЕСЯЧНЫЕ архива= " + date_dd_MM_UU_HH.get(m));

        // формируем timestamp из строки для записи в хеш с полученными часовыми данными

        List<String> d = new ArrayList<String>(Arrays.asList(date_dd_MM_UU_HH.get(m).split(":")));
        ...
        if (type_meas.get(y) == 2) {
            System.out.println(" type_meas.get(y) = " + type_meas.get(y));
            System.out.println("date_dd_MM_UU_HH.get(m) = " +date_dd_MM_UU_HH.get(m));
            System.out.println("timestamp_daily = " +date_dd_MM_UU_HH.get(m));

            mothly_hashMap.put(timestamp_daily, measurementsList);
        }

        if (type_meas.get(y) == 3) {

            System.out.println(" type_meas.get(y) = " + type_meas.get(y));
            System.out.println("date_dd_MM_UU_HH.get(m) = " +date_dd_MM_UU_HH.get(m));
            System.out.println("timestamp_daily = " +date_dd_MM_UU_HH.get(m));

            total_mothly_hashMap.put(timestamp_daily, measurementsList);
        }

        measurementsList=null;
///закрываем цикл for перебора дат для Месячные
    }

///закрываем цикл for перебора типов измерений (2 и 3) для Месячные
}



            /**
             * Закончили сбор данных по
             * type==2  МЕСЯЧНЫЕ
             * type==3  ИТОГОВЫЕ
             *
             * Приступаем к итоговым текущим
             *
             */
 ...
 */  ////////////////////////////////////////////////////////////////////////////////
            ...
            ////(!)(!)(!)(!)/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////(!)(!)(!)(!)//////////////////////Меняем тип запрашиваемго архива на ИТОГОВЫЕ ТЕКУЩИЕ////////////////////////////////////////////////
////(!)(!)(!)(!)//////////////////////type=5 /////////////////////////////////////////////////////////////////////////////////////////
            ...
            //List<String> ff_n=new ArrayList<>();
            System.out.println("\nФормируем новый type= 5 (ИТОГОВЫЕ ТЕКУЩИЕ) 3F FD Запрос на запись ТИПА значений");
 ...
 */
            ...
            System.out.println(" timestamp для ИТОГОВЫЕ ТЕКУЩИЕ измерения = " + timestamp_daily);
                ...
                total_current_hashMap.put(timestamp_daily, measurementsList);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * Фаза получения данных Месячные и итоговые текущие  закончена /////////////////////////////////////////////////////////////////////////////////////////////
...
/*
    ...
    for (Timestamp ts : mothly_hashMap.keySet()) {
        System.out.println("mothly_hashMap TS = "+ ts );

    }
    for (Map.Entry entry :  mothly_hashMap.entrySet()) {
        System.out.println("Key: " + entry.getKey() + " Value: "
                + entry.getValue());
    }
    ...
    System.out.println(ts + " создаем объект operation");
    operation=null;
    operation = new Operation();
        operation.setTypeOperation("moth");
    ...
    System.out.println(" 1");
       // mothly_hashMap.get(ts).forEach(p -> operation.addMeasurements(p));

    for(Measurements measurements: mothly_hashMap.get(ts)){
        operation.addMeasurements(measurements);
    }
    ...
    System.out.println("Запись значений месячных архивов произведена. Записываем итоговые показания");


    for (Timestamp ts : total_mothly_hashMap.keySet()) {
        System.out.println( "total_mothly_hashMap имеет"+ts);
    }
    ...
    for (Timestamp ts : total_mothly_hashMap.keySet()) {
        ...
        operation=null;
           operation = new Operation();
           /* operation2.setTypeOperation("total_mothly");
            operation2.setServerVersion(String.valueOf(server_version));
            operation2.setProgrammVersion(service_information.get(0));
            operation2.setShemaTv13Ff9(service_information.get(1));
            operation2.setTp3Tv1(service_information.get(2));
            operation2.setT5Tv1(service_information.get(3));
            operation2.setShemaTv23Ff9(service_information.get(4));
            operation2.setTp3Tv2(service_information.get(5));
            operation2.setT5Tv2(service_information.get(6));
            operation2.setIdentificator(service_information.get(7));
            operation2.setNetNumber(service_information.get(8));
            operation2.setModel(service_information.get(10));
            operation2.setBeginHourDate(date_3ff6.get(0));
            operation2.setCurrentDate3Ff6(date_3ff6.get(1));
            operation2.setBeginDayDate(date_3ff6.get(2));
            //operation.setDateVkt3Ffb();
            operation2.setDateServer(timestamp);
            operation2.setChronological(ts);
            operation2.setShemaTv13Ecd(String.valueOf(shema_Tb1));
            operation2.setShemaTv23F5B(String.valueOf(shema_Tb2));
            operation2.setBaseNumber(String.valueOf(number_active_base));*/
            operation.setStatus(status);
            operation.setError(String.valueOf(error));
            System.out.println("10 ");
            Thread.sleep(2000);
            //measurementsList.forEach(p -> operation.addMeasurements(p));
           // total_mothly_hashMap.get(ts).forEach(p -> operation2.addMeasurements(p));
            for(Measurements measurements: total_mothly_hashMap.get(ts)){
                operation.addMeasurements(measurements);
                System.out.println(" measurements= "+ measurements);
                //measurements.forEach(p -> System.out.println(p+" "));
            }
    ...
    System.out.println("Запись значений итогового архива произведена. Записываем итоговые текущие");



    for (Timestamp ts : total_current_hashMap.keySet()) {
        System.out.println(ts + " имеет");
        ...
        operation4.setTypeOperation("total_current");
        operation4.setServerVersion(String.valueOf(server_version));
        operation4.setProgrammVersion(service_information.get(0));
        operation4.setShemaTv13Ff9(service_information.get(1));
        operation4.setTp3Tv1(service_information.get(2));
        operation4.setT5Tv1(service_information.get(3));
        operation4.setShemaTv23Ff9(service_information.get(4));
        operation4.setTp3Tv2(service_information.get(5));
        operation4.setT5Tv2(service_information.get(6));
        operation4.setIdentificator(service_information.get(7));
        operation4.setNetNumber(service_information.get(8));
        operation4.setModel(service_information.get(10));
        operation4.setBeginHourDate(date_3ff6.get(0));
        operation4.setCurrentDate3Ff6(date_3ff6.get(1));
        operation4.setBeginDayDate(date_3ff6.get(2));
        //operation.setDateVkt3Ffb();
        operation4.setDateServer(timestamp);
        operation4.setChronological(ts);
        operation4.setShemaTv13Ecd(String.valueOf(shema_Tb1));
        operation4.setShemaTv23F5B(String.valueOf(shema_Tb2));
        operation4.setBaseNumber(String.valueOf(number_active_base));
        operation4.setStatus(status);
        operation4.setError(String.valueOf(error));

        Thread.sleep(2000);
        //measurementsList.forEach(p -> operation.addMeasurements(p));
        total_current_hashMap.get(ts).forEach(p -> operation4.addMeasurements(p));

        Thread.sleep(2000);
        //Customer customer = customerService.findById(id);
        operation4.setIdCustomer(customer.getId());
        operation4.setCustomerName(customer.getFirstName());
        Thread.sleep(2000);
        customer.addOperation(operation4);
        Thread.sleep(2000);
        customer=customerService.save(customer);
    }

    System.out.println("Запись значений итогового текущего произведена. Закрываем подключение");
