...
public class Connect extends EventListener{
    public static SerialPort serialPort; /*Создаем объект типа SerialPort*/
...
/* public volatile static String data2;
   ...
   /*private static int[][] requests = {
    ...
    public static List<Object> connect () throws InterruptedException, TimeoutException, ExecutionException {  /* Точка входа в программу*/
...
*/
/**  //////////////////////////////////////////////////
 * 3F F9 (2) (S15->16->17) Запрос на чтение служебной информации ////
 *//*
  //////////////////////////////////////////////////

            Thread.sleep(3000);
            System.out.println("\n Фoрмируем запрос 3F F9 (2)");
            ff_n=null;
            ff_n= send03Service.s_3FF9("01");
...
//Формируем массив int для отправки в порт
            ...
            System.out.println("\nЖдем получения всех данных после команды 3F F9 (2)");
            executor = Executors.newFixedThreadPool(2);
            t=0;
            repeat=0;
            executor.submit(callable(6));
            ...
            while (recieve_all_byte==1) {
                if (t == 2) {
                    ...
                    if (repeat == 2) {
                        step = 0;
                        executor.shutdown();
                        System.out.println("\n Ответ не поступил 3F F9 (2). Ошибка по таймауту.");
                        Thread.sleep(2000);
                        serialPort.writeBytes("+++".getBytes());
                        serialPort.setFlowControlMode(SerialPort.FLOWCONTROL_RTSCTS_IN |
                                SerialPort.FLOWCONTROL_RTSCTS_OUT);
            ...
            executor.shutdown();
            System.out.println("\nДанные 3F F9 (2)поступили.");
            System.out.println("Ожидаем обработку принятых данных 3F F9(2)");
            ...
            while(step==16){
                ...
                System.out.println("Принятая строка (2) " + data2);
                System.out.println("\n Проверяем контрольную сумму :: " +crc16Service.crc16_valid(new ArrayList<>(Arrays.asList( data2.replace(" ","").split("(?<=\\G.{2})")))));


                List<String> list = recieve03Service.r_3FF9( data2);
                System.out.println("\n Принятый массив 3F F9 (2):: ");
 ...
 *//*
            ...
            while(step==22){
                ...
                //prop_specification = recieve03Service.r_3FFE(data2,prop_session);
                recieve03Service.r_3FFE_Measurements(data2,current_measur);
                step=23;
