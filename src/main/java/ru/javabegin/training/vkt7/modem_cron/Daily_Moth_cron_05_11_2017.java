 ...
 */
@Component
public class Daily_Moth_cron_05_11_2017 extends EventListener_cron {
...
/* public volatile static String data2;
   ...
   /*private static int[][] requests = {
    ...
    public String daily_all_cycle(List<Customer> customerList,CustomerService customerService, OperationService operationService, Date date, int type ) throws InterruptedException, TimeoutException, ExecutionException, SerialPortException {
        ...
        EventListener_cron eL= new EventListener_cron();
 ...
 */
                ...
                while (step == 22) {
                    ...
                    System.out.println("Суточные из daily_hashMap передвычислением итоговых");
                    System.out.println("timestamp_daily для суточного == "+timestamp_daily);
                    List<Measurements> testlist2= new ArrayList<>();
                    testlist2=daily_hashMap.get(timestamp_daily);
                    for(Measurements measurements1:testlist2){
                        System.out.println(measurements1.getText()+" = " +measurements1.getMeasurText()+" байт качества -"+measurements1.getQualityText()+"NS -"+measurements1.getNs());
                ...
                System.out.println("Суточные из daily_hashMap перед записью итоговых");
                System.out.println("timestamp_daily для суточного == "+timestamp_daily);
                List<Measurements> testlist2= new ArrayList<>();
                testlist2=daily_hashMap.get(timestamp_daily);
                for(Measurements measurements1:testlist2){
                ...
                System.out.println("Суточные из daily_hashMap после записи итоговых");
                System.out.println("timestamp_daily для суточного == "+timestamp_daily);
               testlist2= daily_hashMap.get(timestamp_daily);
                for(Measurements measurements1:testlist2){
                    System.out.println(measurements1.getText()+" = " +measurements1.getMeasurText()+" байт качества -"+measurements1.getQualityText()+"NS -"+measurements1.getNs());
...
/*
    ...
    Operation operation = new Operation();
    operation.setTypeOperation("ERROR");
